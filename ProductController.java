package com.gzw.mall.controller.admin;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.fasterxml.jackson.databind.annotation.JsonAppend;
import com.gzw.mall.controller.BaseController;
import com.gzw.mall.entity.*;
import com.gzw.mall.service.*;
import com.gzw.mall.util.OrderUtil;
import com.gzw.mall.util.PageUtil;
...
public class ProductController extends BaseController {
    ...
    public String goToPage(HttpSession session, Map<String,Object> map){
        ...
        List<Category> categoryList=categoryService.select(null,null);
        map.put("categoryList",categoryList);
        logger.info("获取前10条产品列表");
        PageUtil pageUtil=new PageUtil(0,10);
        List<Product> productList=productService.select(null,null,null,pageUtil);
        map.put("productList",productList);
        logger.info("获取产品总数量");
        Integer productCount=productService.selectTotal(null,null);
   ...
   //222
    //按条件查询产品-ajax
    @ResponseBody
    @RequestMapping(value = "admin/product/{index}/{count}",method = RequestMethod.GET,produces = "application/json;char=utf-8")
    public String getProductBySearch(@RequestParam(required = false)String productName /*产品名次*/,
                                     ...
                                     @RequestParam(required = false,defaultValue = "true")Boolean isDesc/*是否降序*/,
        ...
        if(productName!=null){
            //如果为非空字段串则解决中文乱码:URLDecoder.decode(String,"UTF-8)
        ...
        List<Product> productList=productService.select(product,productIsEnabledArray,orderUtil,pageUtil);
        object.put("productList", JSONArray.parseArray(JSON.toJSONString(productList)));
        logger.info("按条件获取产品总数量");
        Integer productCount=productService.selectTotal(product,productIsEnabledArray);
    ...
    public String goToDetailsPage(HttpSession session,Map<String,Object> map,@PathVariable Integer pid/*产品ID*/){
        ...
        List<ProductImage> productImageList=productImageService.select(productId,null,null);
        ...
        List<PropertyValue> propertyValueList=propertyValueService.select(propertyValue,null);
        logger.info("获取产品详情-分类信息对应的属性列表");
        Property property=new Property();
        property.setPropertyCategory(product.getProductCategory());
        List<Property> propertyList=propertyService.select(property,null);
        logger.info("属性值列表和属性值列表和并");
    ...
    @ResponseBody
    @RequestMapping(value = "admin/productImage/{productImageId}",method = RequestMethod.DELETE,produces = "application/json;char=utf-8")
    ...
    public String updateProduct(@RequestParam String productName /*产品名称*/,
        ...
        boolean yn=productService.updateOne(product);
        if(!yn){
            logger.info("产品更新信息失败！事务回滚");
        ...
        if(propertyIdSet.size()>0){
            ...
            yn=propertyValueService.insertList(propertyValueList);
            if(yn){
        ...
        if(propertyIdSet.size()>0){
            ...
            for(int i=0; i<propertyValueList.size(); i++){
                logger.info("正在更新第{}条，",i+1,propertyValueList.size());
                yn=propertyValueService.updateOne(propertyValueList.get(i));
                if(yn){
        ...
        if(propertyDeleteList!=null && propertyDeleteList.length>0){
            ...
            logger.info("共有{}条需删除的产品属性数据",propertyDeleteList.length);
        ...
        if(productSingleImageList!=null && productSingleImageList.length>0){
            ...
            yn=productImageService.insertList(productImageList);
            if(yn){
    ...
    public String getPropertyByCategoryId(@PathVariable Integer propertyCategoryId){
        //封装查询添加
        Category category=new Category();
    ...
    //添加产品信息
    @ResponseBody
    @RequestMapping(value = "admin/product",method = RequestMethod.POST,produces = "application/json;char=utf-8")
    public String addProduct(@RequestParam String productName/*产品名称*/,
                             ...
                             @RequestParam(required = false) String[] productDetailsImageList/*产品详情图片*/){
        JSONObject jsonObject=new JSONObject();
        logger.info("整合产品信息");
        Category category=new Category();
        category.setCategoryId(productCategoryId);
        ...
        logger.info("添加产品信息");
        boolean yn=productService.insertOne(product);
        if(!yn){
        ...
        Set<String> propertySet=object.keySet();
        ...
        if(productSingleImageList!=null && productSingleImageList.length>0){
            ...
            for(String imageName:productSingleImageList){
                ...
                productImageList.add(new ProductImage((byte)0,imageName.substring(imageName.lastIndexOf("/")),product1));
            }
            logger.info("共有{}条产品预览图片数据", productImageList.size());
            yn=productImageService.insertList(productImageList);
            if (yn) {
